...
public sealed class NotesBackupService : INotesBackupService
{
    ...
    private readonly NotesApplicationService _workspaceApplicationService;
    private readonly INoteMediaService _noteMediaService;
    ...
    public NotesBackupService(
        NotesApplicationService workspaceApplicationService,
        INoteMediaService noteMediaService)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            IReadOnlyList<ConvenientNote.Notes.Domain.Notes.Note> importedNotes;
            using (var archive = ZipFile.OpenRead(Path.GetFullPath(packagePath)))
            {
                var readBudget = new ArchiveReadBudget(_limits.MaximumTotalExpandedBytes);
                var validatedArchive = await ValidateArchiveAsync(
    ...
    private static void ValidateNoPreservedIdCollisions(
        ...
        IReadOnlyCollection<ConvenientNote.Notes.Domain.Notes.Note> importedNotes)
    {
