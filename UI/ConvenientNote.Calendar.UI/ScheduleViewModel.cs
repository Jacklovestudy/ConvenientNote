...
public sealed class ScheduleViewModel : BindableBase, INavigationAware, IDisposable
{
    ...
    public ScheduleViewModel(CalendarApplicationService service)
    {
        _service = service;
        ...
        AddTaskCommand = new DelegateCommand(async () => await AddTaskAsync(), () => _service.SupportsTodos && !IsBusy && !string.IsNullOrWhiteSpace(NewTitle));
        AddEventCommand = new DelegateCommand(async () => await AddEventAsync(), () => !IsBusy && !string.IsNullOrWhiteSpace(NewEventTitle));
        DeleteEventCommand = new DelegateCommand<CalendarTaskViewModel>(async item =>
        {
            if (item is { IsEvent: true }) await WriteAsync(() => _service.DeleteEventAsync(item.Id));
        }, item => !IsBusy && item is { IsEvent: true });
        ...
        _service.Changed += OnWorkspaceChanged;
        Rebuild();
    }
    public event EventHandler? DesktopModeRequested;
    public event EventHandler? CalendarPositionReset;
    public string ViewTitle => "日历";
    public string ViewDescription => "日程安排与待办日历";
    public bool SupportsTodos => _service.SupportsTodos;
    public string NewEventTitle { get => _newEventTitle; set { SetProperty(ref _newEventTitle, value); AddEventCommand.RaiseCanExecuteChanged(); } }
    ...
    public DelegateCommand<CalendarTaskViewModel> DeleteEventCommand { get; }
    ...
    public bool IsBusy { get => _isBusy; private set { SetProperty(ref _isBusy, value); AddTaskCommand.RaiseCanExecuteChanged(); AddEventCommand.RaiseCanExecuteChanged(); DeleteEventCommand.RaiseCanExecuteChanged(); ToggleCompletionCommand.RaiseCanExecuteChanged(); SaveDateCommand.RaiseCanExecuteChanged(); ScheduleUnassignedCommand.RaiseCanExecuteChanged(); } }
    ...
    public async Task RefreshAsync()
    {
        ...
        try
        {
            _notes = await _service.ListAsync();
    ...
    public Task AddTaskAsync() => WriteAsync(async () =>
    {
        var title = NewTitle.Trim();
        if (title.Length == 0) return;
        await _service.CreateTodoAsync(title, SelectedDate);
        NewTitle = "";
    });
    public Task AddEventAsync() => WriteAsync(async () =>
    {
        ...
        if (!IsAllDay)
        {
            if (!TimeOnly.TryParseExact(StartTime, "HH:mm", CultureInfo.InvariantCulture, DateTimeStyles.None, out var from)
                ...
                throw new ArgumentException("请输入 HH:mm 格式的时间，例如 09:30。");
            start = SelectedDate.Add(from.ToTimeSpan());
            end = SelectedDate.Add(to.ToTimeSpan());
        }
        await _service.CreateEventAsync(NewEventTitle, start, end, IsAllDay);
        NewEventTitle = "";
    });
    public Task ToggleCompletionAsync(CalendarTaskViewModel task) => WriteAsync(() => _service.SetCompletionAsync(task.Note, !task.IsCompleted));
    public Task RescheduleAsync(CalendarTaskViewModel task, DateTime? date) => WriteAsync(() => _service.RescheduleAsync(task.Note, date?.Date));
    ...
    {
        var tasks = _tasks = _notes.Select(n => new CalendarTaskViewModel(n)).OrderBy(t => t.IsCompleted).ThenBy(t => t.Note.PlannedDate).ToArray();
    ...
    private CalendarDayViewModel CreateDay(DateTime date) => new(date, true, date == DateTime.Today,
        date == SelectedDate, CalendarDateInfo.Label(date), _tasks.Where(t => t.Note.OccursOn(date)).ToArray());
    ...
    public void Dispose() => _service.Changed -= OnWorkspaceChanged;
}
