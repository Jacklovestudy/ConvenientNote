...
namespace ConvenientNote.ViewModels
{
    ...
    public abstract class TodoBoardViewModel : BindableBase, INavigationAware
    {
        ...
        private readonly TodoApplicationService _todoApplicationService;
        private readonly ITodoWeatherService _weatherService;
        ...
        private readonly HashSet<TodoId> _deletingTodoIds = new();
        ...
        protected TodoBoardViewModel(TodoApplicationService todoApplicationService, ITodoWeatherService weatherService, string boardKey, TodoBoardFilter filter, string viewTitle, string viewDescription, bool canAddTodo, bool filterByDate = false)
        {
            ...
            AddTodoCommand = new DelegateCommand(async () => await AddTodoAsync(), () => CanAddTodo);
        ...
        public double BoardWidth { get => _boardWidth; private set => SetProperty(ref _boardWidth, value); }
        public double BoardHeight { get => _boardHeight; private set => SetProperty(ref _boardHeight, value); }
        public string CurrentDateTitle { get => _currentDateTitle; private set => SetProperty(ref _currentDateTitle, value); }
        public string CalendarMonthTitle { get => _calendarMonthTitle; private set => SetProperty(ref _calendarMonthTitle, value); }
        public string QuickAddTitle { get => _quickAddTitle; set => SetProperty(ref _quickAddTitle, value); }
        public string Summary { get => _summary; private set => SetProperty(ref _summary, value); }
        public string EmptyStateTitle { get => _emptyStateTitle; private set => SetProperty(ref _emptyStateTitle, value); }
        public string EmptyStateDescription { get => _emptyStateDescription; private set => SetProperty(ref _emptyStateDescription, value); }
        public string WeatherText { get => _weatherText; private set => SetProperty(ref _weatherText, value); }
        public string WeatherToolTip { get => _weatherToolTip; private set => SetProperty(ref _weatherToolTip, value); }
        public PackIconKind WeatherIconKind { get => _weatherIconKind; private set => SetProperty(ref _weatherIconKind, value); }
        public Visibility EmptyStateVisibility => TodoItems.Count == 0 ? Visibility.Visible : Visibility.Collapsed;
        ...
        {
            if (_deletingTodoIds.Contains(todo.Id) || _currentWorkspaceId is not { } workspaceId)
            {
                return;
            }

            await _todoApplicationService.RenameAsync(workspaceId, todo.Id, todo.Title);
        }
        ...
        public async Task CommitTodoContentAsync(CanvasTodoViewModel todo)
        {
            ...
            await _todoApplicationService.UpdateContentAsync(workspaceId, todo.Id, todo.Content);
        }
        ...
        public async Task CommitTodoPriorityAsync(CanvasTodoViewModel todo)
        {
            ...
            await _todoApplicationService.SetPriorityAsync(workspaceId, todo.Id, todo.Priority);
        }
        ...
        public async Task CommitTodoPositionAsync(CanvasTodoViewModel todo)
        {
            ...
            await _todoApplicationService.MoveAsync(workspaceId, todo.Id, todo.X, todo.Y);
            RefreshBoardSize();
        }

        public async Task DeleteTodoAsync(CanvasTodoViewModel todo)
        {
            if (_currentWorkspaceId is not { } workspaceId || !_deletingTodoIds.Add(todo.Id))
            {
                return;
            }
            ...
            try
            {
                await _todoApplicationService.DeleteAsync(workspaceId, todo.Id);
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> ArrangeTodosAsync(double viewportWidth)
        {
            ...
            var originalPositions = TodoItems.Select(todo => new TodoPositionUpdate(todo.Id, todo.X, todo.Y)).ToList();
            ...
            var orderedTodos = TodoItems.OrderBy(todo => Math.Round(todo.Y / rowHeight)).ThenBy(todo => todo.X).ThenBy(todo => todo.Y).ToList();
            ...
            var columnCount = Math.Max(1, (int)Math.Floor((usableViewportWidth - (InitialTodoOffset * 2) + TodoHorizontalGap) / columnWidth));
            var arrangedPositions = new List<TodoPositionUpdate>(orderedTodos.Count);
            for (var index = 0; index < orderedTodos.Count; index++)
            {
                ...
                todo.MoveTo(x, y);
                arrangedPositions.Add(new TodoPositionUpdate(todo.Id, x, y));
            }

            RefreshBoardSize();
            try
            {
                await _todoApplicationService.MoveManyAsync(workspaceId, arrangedPositions);
                return true;
            }
            catch (Exception ex)
            {
                ...
                {
                    var todo = TodoItems.FirstOrDefault(current => current.Id == originalPosition.TodoId);
        ...
        private async Task LoadWorkspaceAsync()
        {
            try
            {
                var workspace = await _todoApplicationService.GetCurrentAsync();
                _currentWorkspaceId = workspace.Id;
                _allTodoItems.Clear();
                foreach (var note in workspace.Todos)
                {
        ...
        private async Task AddTodoAsync()
        {
            ...
            var x = latestTodo is null ? InitialTodoOffset : latestTodo.X + latestTodo.Width + TodoHorizontalGap;
            var y = latestTodo?.Y ?? InitialTodoOffset;
            var note = _filterByDate ? await _todoApplicationService.CreateTodoAsync(workspaceId, x, y, title, _selectedDate) : await _todoApplicationService.CreateTodoAsync(workspaceId, x, y, title);
            _allTodoItems.Add(CreateTodoViewModel(note));
        ...
        private CanvasTodoViewModel CreateTodoViewModel(TodoItem note)
        {
            return new CanvasTodoViewModel(note, async todo =>
            {
                if (_currentWorkspaceId is { } workspaceId)
                {
                    await _todoApplicationService.SetCompletionAsync(workspaceId, todo.Id, todo.IsCompleted);
                    await LoadWorkspaceAsync();
                }
            });
        }
        ...
        private void RefreshVisibleTodos()
        {
            ...
            if (_filterByDate)
                boardTodos = boardTodos.Where(todo => todo.PlannedDate == _selectedDate);
        ...
        private void RefreshBoardSize()
        {
            ...
            BoardWidth = Math.Max(MinimumBoardWidth, TodoItems.Max(todo => todo.X + todo.Width + BoardContentPadding));
            BoardHeight = Math.Max(MinimumBoardHeight, TodoItems.Max(todo => todo.Y + todo.Height + BoardContentPadding));
        }
        ...
        private void RefreshViewStatus()
        {
            ...
            Summary = _filter == TodoBoardFilter.Completed ? $"已完成 {completedCount} / 全部 {totalCount}" : $"{ViewTitle} · {TodoItems.Count} 项";
        ...
        private void RefreshDateStrip()
        {
            ...
            for (var index = 0; index < 7; index++)
            {
                ...
                DateTabs.Add(new DateTabViewModel(date, date == DateTime.Today, date == _selectedDate));
            }
        ...
        {
            var firstPart = locationName.Split('·', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries).FirstOrDefault();
            return string.IsNullOrWhiteSpace(firstPart) ? "当前位置" : firstPart;
        }
    }
}
